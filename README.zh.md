...
只有当当前会话指向 MiniMax 端点时,status line 才会渲染 `MiniMax` 行,
才会调用 API、读写缓存。检测逻辑是读取 `ANTHROPIC_BASE_URL`,将其
hostname 与 `minimaxi.com` / `minimax.com` 匹配(大小写不敏感,包含子域名)。

| `ANTHROPIC_BASE_URL` | `MiniMax` 行 | 网络请求 |
| --- | --- | --- |
| 未设置或为空 | 隐藏 | 跳过 |
| 非 MiniMax 主机(如 `https://api.anthropic.com`) | 隐藏 | 跳过 |
| `https://www.minimaxi.com` 或任何 `*.minimaxi.com` / `*.minimax.com` | 显示 | 发起 |

非法 URL(如 `not-a-url`)按非 MiniMax 处理,该行隐藏——不会崩溃,也不会发出网络请求。
