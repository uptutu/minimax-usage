...
export function renderTimeProgress(
  ...
  if (process.env.HUD_LEGACY_TIME === '1') return '';
 ...
 */
export function sumContextTokens(cu: unknown): number | null {
  ...
  for (const f of fields) {
    const v = (cu as Record<string, unknown>)[f];
...
function getContextBar(usedPercent: number | null | undefined, width: number = 10, usedTokens: number | null = null, totalTokens: number | null = null): string {
...
export function renderProvider(data: NormalizedUsage | null, stdin: StdinData = {}): void {
  ...
  if (hasInterval && !hasWeekly) {
    ...
    if (intervalTime) console.log(`  ${label} │ 5h  ${intervalTime}`);
    return;
  }
  ...
  if (!hasInterval) {
    ...
    console.log(
      `  ${label} │ 5h  ${empty5hBar} ─ (100%) │ 7d ${weeklyBar} ${formatPercent(weeklyUsed)}% (${formatPercent(totalPercent)}%)${resetStr}${weeklyTimeOnly ? ' ' + weeklyTimeOnly : ''}`
    );
    return;
  }
